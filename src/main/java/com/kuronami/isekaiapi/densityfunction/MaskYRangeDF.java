 ...
 */
@ApiStatus.Internal
public record MaskYRangeDF(int ymin, int ymax, DensityFunction inside, DensityFunction outside) implements CompositeDensityFunction {
    ...
    static final KeyDispatchDataCodec<MaskYRangeDF> KEY_CODEC = new KeyDispatchDataCodec<>(CODEC);

    @Override public double compute(FunctionContext ctx) {
        int y = ctx.blockY();
        return (y >= ymin && y <= ymax) ? inside.compute(ctx) : outside.compute(ctx);
    }
    @Override public DensityFunction mapAll(Visitor v) { return v.apply(new MaskYRangeDF(ymin, ymax, inside.mapAll(v), outside.mapAll(v))); }
