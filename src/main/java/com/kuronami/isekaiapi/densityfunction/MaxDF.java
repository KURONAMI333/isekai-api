...
@ApiStatus.Internal
public record MaxDF(DensityFunction a, DensityFunction b) implements CompositeDensityFunction {
    ...
    static final KeyDispatchDataCodec<MaxDF> KEY_CODEC = new KeyDispatchDataCodec<>(CODEC);

    @Override public double compute(FunctionContext ctx) { return Math.max(a.compute(ctx), b.compute(ctx)); }
    @Override public DensityFunction mapAll(Visitor v) { return v.apply(new MaxDF(a.mapAll(v), b.mapAll(v))); }
