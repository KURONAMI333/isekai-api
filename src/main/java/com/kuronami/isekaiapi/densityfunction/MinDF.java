...
@ApiStatus.Internal
public record MinDF(DensityFunction a, DensityFunction b) implements CompositeDensityFunction {
    ...
    static final KeyDispatchDataCodec<MinDF> KEY_CODEC = new KeyDispatchDataCodec<>(CODEC);

    @Override public double compute(FunctionContext ctx) { return Math.min(a.compute(ctx), b.compute(ctx)); }
    @Override public DensityFunction mapAll(Visitor v) { return v.apply(new MinDF(a.mapAll(v), b.mapAll(v))); }
